 ...
 */
    ...
    function generateFinalSchema(entities, primaryType, pageUrl) {
        ...
        entities.forEach(entity => {
            ...
            // تجاهل أجزاء العنوان لأنها ليست كيانات مستقلة، بل بيانات وصفية
            // سيتم استخدامها داخل populateJobPostingProperties.
            if (entity.type === 'JobPosting:AddressPart') {
                return;
            }
    ...
    function populateJobPostingProperties(schema, entities, isPrimary) {
        ...
        // --- 4. Definitive Hybrid and Intelligent Salary Handling ---
        const salaryEntity = findJobEntity('baseSalary');
        let numericSalaryValue = null;
        let detectedUnitText = "YEAR";
        let detectedCurrency = findJobEntity('currency')?.value || "USD";
        const manualSalaryOverride = DOM.customJobSalaryValue.value.trim();
        if (manualSalaryOverride) {
            numericSalaryValue = manualSalaryOverride;
        } else if (salaryEntity?.value) {
            const numericMatch = salaryEntity.value.match(/(\d[\d,.]*)/);
            if (numericMatch) numericSalaryValue = numericMatch[0].replace(/,/g, '');
        }
        if (numericSalaryValue && salaryEntity?.value) {
        ...
        if (numericSalaryValue) {
            schema.baseSalary = {
                "@type": "MonetaryAmount",
                "currency": detectedCurrency,
                "value": { "@type": "QuantitativeValue", "value": numericSalaryValue, "unitText": detectedUnitText }
            };
        }
    ...
    function getPublisherData(entities) {
        ...
        if (empTelephone) {
            finalTelephone = empTelephone;
        } else {
            const telephoneEntity = entities.find(e => e.type === 'Organization' && e.schemaProp === 'telephone');
            if (telephoneEntity) {
                finalTelephone = telephoneEntity.value;
            }
        }

        if (finalTelephone) {
            orgData.telephone = finalTelephone;
    ...
    async function handleAnalysis() {
        ...
        try {
            ...
            // --- Pre-fill Smart UI with discovered data (Expert Version) ---
            // This block now coordinates the entire Job Posting UI based on all discovered entities.

            // A. Location Pre-fill Logic
            const locationTypeEntity = entities.find(e => e.schemaProp === 'jobLocationType');
            const locationType = locationTypeEntity?.value; // Can be 'remote', 'hybrid', or 'physical'

            const findAddressPart = (prop) => entities.find(e => e.type === 'JobPosting:AddressPart' && e.schemaProp === prop);

            if (locationType === 'remote') {
                DOM.customJobLocationType.value = 'remote';
                ...
                // For both physical and hybrid jobs, we show the physical address fields.
                // For hybrid, the title will be adjusted later in the populate function.
                DOM.customJobLocationType.value = 'physical';
    ...
    if (DOM.customJobLocationType) {
        DOM.customJobLocationType.addEventListener('change', (e) => {
            ...
            DOM.physicalLocationFields.style.display = isRemote ? 'none' : 'block';
            DOM.remoteLocationFields.style.display = isRemote ? 'block' : 'none';

            // **PROFESSIONAL POLISH:** When hiding a section, clear its input values.
            // This prevents old data from persisting incorrectly if the user switches back and forth.
            if (isRemote) {
                // Clear physical fields when switching to remote
                ...
                // Clear remote field when switching to physical
