            ...
            <div class="card shadow-lg border-0" style="backdrop-filter: blur(10px); background: rgba(var(--bs-body-bg-rgb), 0.95);">
                <div class="card-header d-flex justify-content-between align-items-center py-2">
                    <small class="fw-bold text-muted">
                        <i class="bi bi-hdd-stack me-1"></i>Storage Usage
                    </small>
                    <button type="button" class="btn-close btn-close-sm" id="storage-monitor-close"></button>
                </div>
                <div class="card-body p-3">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-danger w-100 mt-2" id="storage-cleanup-btn">
    ...
    function performCleanup() {
        ...
        if (result.success) {
            ...
            if (stats.status === 'critical' || stats.status === 'warning') {
                if (hardResetModalInstance) {
                    hardResetModalInstance.show();
                }
            } else {
                showToast('No items needed cleaning.', 'info');
            }
        }
    }
