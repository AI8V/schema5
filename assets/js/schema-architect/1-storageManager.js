...
const StorageManager = (function() {
    ...
    function getStorageStatus() {
        ...
        if (percent >= CONFIG.QUOTA_CRITICAL_THRESHOLD) {
            return { status: 'critical', percent, needsCleanup: true };
        } else if (percent >= CONFIG.QUOTA_WARNING_THRESHOLD) {
            return { status: 'warning', percent, needsCleanup: false };
        }
    ...
    function shouldCompress(data) {
        ...
        return size >= CONFIG.MIN_SIZE_FOR_COMPRESSION;
    }
    ...
    function getMetadata() {
        try {
            const meta = localStorage.getItem(CONFIG.KEYS.METADATA);
    ...
    function updateMetadata(key, type) {
        ...
        localStorage.setItem(CONFIG.KEYS.METADATA, JSON.stringify(meta));
    }
    ...
    function cleanupCache() {
        const meta = getMetadata();
        const cacheEntries = Object.entries(meta.cache)
            .sort((a, b) => a[1].lastAccessed - b[1].lastAccessed);
        
        let cleaned = 0;
        for (let i = 0; i < Math.min(CONFIG.CLEANUP_BATCH_SIZE, cacheEntries.length); i++) {
    ...
    function cleanupProjects() {
        try {
            const projectsData = JSON.parse(localStorage.getItem(CONFIG.KEYS.PROJECTS) || '{}');
            const meta = getMetadata();
            ...
            const projectEntries = Object.entries(projectsData)
                .map(([name, data]) => ({
                    name,
                    data,
                    lastAccessed: meta.projects[name]?.lastAccessed || 0
                }))
                .sort((a, b) => b.lastAccessed - a.lastAccessed);
            
            if (projectEntries.length <= CONFIG.MAX_PROJECTS) {
                return 0;
            }
            
            const toKeep = projectEntries.slice(0, CONFIG.MAX_PROJECTS);
            const newProjectsData = {};
            
            toKeep.forEach(({ name, data }) => {
                newProjectsData[name] = data;
            });
            
            localStorage.setItem(CONFIG.KEYS.PROJECTS, JSON.stringify(newProjectsData));
            
            // Update metadata
            const newMeta = getMetadata();
            newMeta.projects = {};
            toKeep.forEach(({ name }) => {
                if (meta.projects[name]) {
                    newMeta.projects[name] = meta.projects[name];
                }
            });
            localStorage.setItem(CONFIG.KEYS.METADATA, JSON.stringify(newMeta));
            
            return projectEntries.length - toKeep.length;
        } catch (error) {
    ...
    function performEmergencyCleanup() {
        ...
        return {
            success: cleanedCache > 0 || cleanedProjects > 0,
            cache: cleanedCache,
        ...
        try {
            ...
            if (error.name === 'QuotaExceededError') {
                ...
                if (cleanup.success) {
                    ...
                    try {
                        const { compressed, data: processedData } = compressData(data);
                        localStorage.setItem(key, JSON.stringify({ compressed, data: processedData }));
    ...
    return {
        ...
        // Direct access to keys (for backwards compatibility)
        KEYS: CONFIG.KEYS
    };
